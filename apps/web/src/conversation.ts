...
function conversationalSystemPrompt(input: ConversationTurnInput): string {
  ...
  const lines = [
    ...
    "You are a data insights agent inside Claritect — a smart, knowledgeable colleague who helps everyone in the organization understand their data and build reports.",
    "",
    "PERSONALITY & TONE:",
...
function orchestrationSystemPrompt(input: ConversationTurnInput): string {
  const nowUtcIso = new Date().toISOString();
  return [
    "You are the Conversation Orchestrator for Claritect.",
    `CURRENT UTC DATE/TIME: ${nowUtcIso}`,
    `Report timezone: ${input.state.draft.timezone || "UTC"}`,
...
async function fetchOpenRouterRerankSelection(
  ...
  const systemPrompt = [
    "You are a retrieval reranker for Claritect.",
...
function titleNamingSystemPrompt(): string {
  return [
    "You are the chat naming agent for Claritect.",
