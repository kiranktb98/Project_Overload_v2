  ...
  <head>
    ...
    <style>
      ...
      *{box-sizing:border-box} body{margin:0;min-height:100vh;font-family: Inter, "Sohne", "Suisse Intl", sans-serif;color:var(--ink);background:radial-gradient(circle at 14% 10%,rgba(108,58,237,.22),transparent 24%),radial-gradient(circle at 88% 8%,rgba(236,72,153,.15),transparent 26%),linear-gradient(180deg,#0F0B1A 0%,#130F20 44%,#161122 100%)}
      ...
      .page{padding:14px}.layout{display:grid;grid-template-columns:212px 1fr;height:calc(100vh - 28px);gap:14px}
      ...
      .content-shell{background:linear-gradient(180deg,rgba(20,15,34,.98),rgba(17,12,28,.99));overflow-y:auto}.workspace{display:grid;grid-template-columns:minmax(0,1.45fr) minmax(360px,1fr);gap:16px;padding:18px;align-items:start}
    ...
    <div class="page">
      <div class="layout">
        <aside class="platform-panel">
          ...
          <nav class="platform-nav">
            <a class="platform-link" href="/connect/tls-guide" target="_blank" rel="noreferrer"><svg viewBox="0 0 24 24"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>SSL / TLS Guide</a>
          </nav>
