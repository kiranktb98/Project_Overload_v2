...
function queryStrategistSystemPrompt(input: QueryStrategyInput): string {
  ...
  return [
    ...
    "For each business question, you must prepare a complete dataset by writing 1-3 focused SQL queries.",
    ...
    "",
    "For EACH business question, ask yourself:",
    "- Does the analyst have the raw numbers to compute trends? (e.g., monthly totals over time)",
    "- Does the analyst have comparison data? (e.g., this period vs last period)",
    "- Does the analyst have dimensional breakdowns? (e.g., by product, by region, by status)",
    "- Does the analyst have the totals and averages needed for executive insights?",
    "",
    "If one query can't cover all that, USE MULTIPLE QUERIES with the same group_id.",
    "",
    ...
    "",
    "For a typical business question, prepare 2-3 queries:",
    "  Query 1: The primary aggregation (e.g., monthly revenue totals, top-N products)",
    "  Query 2: A complementary breakdown (e.g., by category, by region, by status)",
    "  Query 3: A comparison or context query (e.g., previous period, year-ago data, overall averages)",
    "",
    "WHEN TO USE 1 QUERY: Simple factual questions ('total revenue last month', 'count of active users')",
    "WHEN TO USE 2 QUERIES: Questions needing trend + breakdown ('revenue trend by category')",
    "WHEN TO USE 3 QUERIES: Complex diagnostics ('why did revenue drop?' needs: trend, breakdown, comparison)",
    "",
...
function queryStrategistUserPrompt(input: QueryStrategyInput): string {
  ...
  parts.push("Generate 2-4 business questions. For EACH question, prepare 1-3 SQL queries sharing the same group_id.");
